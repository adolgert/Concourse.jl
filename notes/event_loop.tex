...
Scope: draw-free models only, refused at factory construction. Branch
worlds run forward past any record, and a model whose \code{fire} consumes
auxiliary randomness would need live keyed draw streams with clone/rekey
semantics of their own — an open design, not an accident. The live model
binding (\code{live\_model}) gives SPA's twin an off-record \code{fire}
that fails loudly if a draw is requested.
...
occupancy. SPA on PS remains refused upstream by its own multi-segment
guard (the boundary weight for a re-declared candidate is unprototyped),
and IPA over PS records is untested — the two honest residues.
