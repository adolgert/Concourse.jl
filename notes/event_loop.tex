...
Implemented July 17, 2026. Three findings, one of them a deliberate
departure from \code{queue\_layers.tex}.

\textbf{The anchor convention.} \code{queue\_layers.tex}~\S3.5 compiled a
speed change as a re-enabling \emph{anchored at the current time}, with the
banked age moved into a left truncation and an explicit warning against
also keeping the original enabling time. Concourse does the opposite:
\code{te} stays at the original enabling forever, and a speed change is a
\emph{mid-flight re-evaluation} of the clock's wall-time law
\[
\text{age} \;\sim\; \text{shift} + (X - a)/r
\;\Big|\; X > a,
\qquad X \sim F,\; \text{shift} = \text{anchor} - t_e,
\]
carried by the state's \code{bank} (internal age $a$ at the last change)
and \code{anchor} (that change's wall time) — an A1 addendum. The reason is
the ClockGradients contract: the Bookkeeper reconstructs \code{te} as the
enabling firing's time, the score replay accumulates piecewise survival
against distributions anchored at that fixed \code{te}, and the segment
machinery (CG-M3) exists precisely to represent re-evaluations at constant
anchor. Under the anchor-at-now convention every speed change would
invalidate the reconstructed \code{te} and the whole estimation stack;
under the fixed-anchor convention it composes with no changes at all. The
sampler side is the age-keeping \code{reenable!} (\code{relative\_te =
te − now} $\le 0$), which CompetingClocks documents as exactly this case.
The interpreter did not change: the \code{:reenable} branch it already had
for D1 computes that shift.
